...
let writeDotFile<'a> (ga: GraphvizAlgorithm<'a, Edge<'a>>, fileName: string) =
    ga.Generate(new FileDotEngine(), fileName) |> ignore
...
let ``Can access properties of generative provider`` () =
    ...
    let udf =
        Example.UserDefinedField(name = "Score", value = "1"),
        [ Example.UserDefinedField(name = "Test", value = "Passed")
          Example.UserDefinedField(name = "Core", value = "Seeds") ]

    let s =
        Example.SecurityResponse(currency = Some Example.Currency.CAD, userDefined = udf, name = "AAPL US Equity")
...
let ``Write Schemas As Dot Files`` () =
    ...
    |> List.iter (fun schemaPath ->
        ...
        writeDotFile (
            toGraphviz (graph),
            Path.Combine(Path.GetDirectoryName(schemaPath), Path.GetFileNameWithoutExtension(schemaPath))
        ))
